...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	for(int i = 1; i < argCount; i++)
		if(!wcstombs_s(&ret, buf, argLength, argList[i], _TRUNCATE))
			args.push_back(std::string(buf));

    CBackgroundRemovalBasics application(args);
...
HRESULT  CBackgroundRemovalBasics::ComposeImage()
{
	...
	nManager->updateBuffer(static_cast<int>(m_colorWidth), static_cast<int>(m_colorHeight), cBytesPerPixel, pBackgroundRemovedColor);
